...
os.makedirs("./shapenet_generate", exist_ok=True)
...
def do_job(i):      #处理函数  处理第i个模型
    ...
    worker(i, 100, 20, good_grasp)   # 100 20
    ...
    good_grasp_file_name = "./shapenet_generate/{}_{}_{}".format(filename_prefix, str(object_name), str(len(good_grasp)))
...
def worker(i, sample_nums, grasp_amount, good_grasp):  #主要是抓取采样器以及打分    100  20
    ...
    print("Log: do job", i)
    #设置obj模型文件与sdf文件路径
    if os.path.exists(str(file_list_all[i]) + "/models/model_normalized.obj") and os.path.exists(str(file_list_all[i]) + "/models/model_normalized.sdf"):
        of = ObjFile(str(file_list_all[i]) + "/models/model_normalized.obj")
        sf = SdfFile(str(file_list_all[i]) + "/models/model_normalized.sdf")
    else:
        print("can't find any obj or sdf file!", file_list_all[i])
        return
...
#########################################
    ...
    if less_class:
        ...
        fc_list = np.concatenate([fc_list_sub1, fc_list_sub2])  # [2. 1.7 1.4 1.1 0.8 0.5 0.4]
    print("判断摩擦系数")
    print(fc_list)
    ...
    p_good_count_perfect = np.zeros(len(fc_list))

    count = 0
    #设置每个摩擦值需要计算的最少抓取数量 （根据指定输入值20）
    minimum_grasp_per_fc = grasp_amount
    # grd_list =np.loadtxt('/home/aaronstc/code/PointNetGPD/PointNetGPD/data/ycb-tools/models/shapenet/2b28e2a5080101d245af43a64155c221/models/model_normalized_sdf_grd_list.txt').astype(np.int32)
    # grd_list =np.loadtxt('/home/aaronstc/code/PointNetGPD/PointNetGPD/data/ycb-tools/models/shapenet/1a74b169a76e651ebc0909d98a1ff2b4/models/model_normalized_sdf_grd_list.txt').astype(np.int32)
    grd_list =np.loadtxt('/home/aaronstc/code/PointNetGPD/PointNetGPD/data/ycb-tools/models/shapenet/2b28e2a5080101d245af43a64155c221/models/model_normalized_sdf_grd_list.txt').astype(np.int32)
    ...
    while np.sum(good_count_perfect < minimum_grasp_per_fc) != 0:    
        ...
        grasps = ags.generate_grasps(obj, target_num_grasps=sample_nums, grasp_gen_mult=10, max_iter=5,
                                     vis=False, random_approach_angle=True, grd_list=grd_list)
        count += len(grasps)
        last_p_good_count_perfect = copy.deepcopy(p_good_count_perfect)
        ...
        for idx in range(good_count_perfect.shape[0]):
            p_good_count_perfect[idx] = min(good_count_perfect[idx], minimum_grasp_per_fc)  # 20
        if np.sum(p_good_count_perfect == last_p_good_count_perfect) == p_good_count_perfect.shape[0]:
            patient = patient + 1
        else:
            patient = 0
        # print(p_good_count_perfect, last_p_good_count_perfect, patient)
        print('Current:{} Last:{} Patient:{}/6'.format(p_good_count_perfect, last_p_good_count_perfect, patient))
...
if __name__ == '__main__':
    ...
    #存放CAD模型的文件夹
    file_dir = home_dir + "/PointNetGPD/data/ycb-tools/models/shapenet"   #获取模型的路径
    # file_dir = home_dir + "/PointNetGPD/data/ycb-tools/models/shapenet_mesh"   #获取模型的路径

...
def open_npy_and_obj(name_to_open_):
    ...
    object_name_ = name_to_open_.split("/")[-1].split("_")[1]


    print(object_name_)
    ply_name_ = file_dir + object_name_ + "/models/model_normalized.obj"  # .obj  and .ply is ok
    if not check_pcd_grasp_points:
        of = ObjFile(file_dir + object_name_ + "/models/model_normalized.obj")
        sf = SdfFile(file_dir + object_name_ + "/models/model_normalized.sdf")
        mesh = of.read()
        sdf = sf.read()
...
def show_selected_grasps_with_color(m, ply_name_, title, obj_):
    ...
    if save_fig or show_fig:
        ...
        if save_fig:
            ...
            mlab.title("good_"+title, size=0.2)
        ...
        if save_fig:
            ...
            mlab.title("bad_"+title, size=0.5)
            mlab.show()
...
if __name__ == "__main__":
    ...
    npy_names = glob.glob(home_dir + "/dex-net/apps/shapenet_generate/*.npy")

    npy_names.sort()
    print(home_dir + "/PointNetGPD/dex-net/apps/shapenet_generate/*.npy")

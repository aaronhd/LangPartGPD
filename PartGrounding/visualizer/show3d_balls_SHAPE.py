...
def showpoints(xyz, part_gt=None, part_pred=None, waittime=0, showrot=False, magnifyBlue=0, freezerot=False,
    ...
    cmap = np.array([[1.00000000e+00, 1.00000000e+00, 1.00000000e+00],
    ...
    if part_gt is not None:
        c_gt = cmap[part_gt, :]
    else:
        c_gt = None

    if part_pred is not None:
        c_pred = cmap[part_pred, :]
    else:
        c_pred = None
    ...
    def render():
        ...
        cv2.putText(show, 'press P prediction, T groundtruth' , (10, 20), 0, 0.5,
                        (255, 0, 0))
        if magnifyBlue > 0:
...
if __name__ == '__main__':
    ...
    data = dataset[idx]
    point_set, label, seg, sent, _ = data
    choice = np.random.choice(point_set.shape[0], opt.npoints, replace=True)
    point_set, seg = point_set[choice, :], seg[choice]
    seg = seg - seg.min()

    print(sent)
    showpoints(point_set, part_gt=seg, part_pred=seg, waittime=0, showrot=False, magnifyBlue=0, freezerot=False,
